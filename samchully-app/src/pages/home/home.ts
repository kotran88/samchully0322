...
export class HomePage {
      ...
      for (var engineer in sn.val()) {
        ...
        /*
          ...
          */
          ...
          console.log("this.engineer_id.split == " + this.engineer_id.split("@")[0]);
          console.log(this.agency_name);
          if(engineer == this.engineer_id.split("@")[0]) {
            // if(sn.val()[engineer].authority == "대리점대표") {
              console.log('대리점 대표로 로그인하면 여기 탄다.');
              for(var engineer in sn.val()){
                console.log(engineer,sn.val()[engineer].agency)
                if(sn.val()[engineer].agency == this.agency_name) {
                  console.log('ok')
                  for (var list in sn.val()[engineer].currentReceipt) {
                    ...
                    if(sn.val()[engineer].currentReceipt[list].status == "접수" || sn.val()[engineer].currentReceipt[list].status == "처리진행" || sn.val()[engineer].currentReceipt[list].status == "승인완료" || sn.val()[engineer].currentReceipt[list].status == "승인보류") {
            ...
            // }
            // else {
              // if(sn.val()[engineer].authority != "기사권한"){
              //   this.firemain.child('engineer').child(engineer).update({authority:"기사권한"});
              // }
              // console.log('기사로 로그인하면 여기 탄다.');
              // // 해당 기사가 배정받은 정보만을 가져온다.
              // // this.engineer_id.split("@")[0]
              // for (var list in sn.val()[this.engineer_id.split("@")[0]].currentReceipt) {
          ...
          console.log(this.as_complete_data);
          console.log(this.as_hold_data);
          console.log(':::1');
        }
      }
    }).then(() => {
  ...
  flag2_save() {
    ...
    if(this.flag6_store==""){
      window.alert("접수고객을22 입력해주세요.");
      this.flag=6;
      return;
    }
    else if (this.flag6_name == ""){
      // window.alert("접수 고객 이름을 입력해주세요.");
      // this.flag=6;
      // return;
    }
    else if (this.flag6_address == ""){
      window.alert("주소를 입력해주세요.");
      this.flag=6;
      return;
    }
    else if(this.flag6_engineer==""){
      // window.alert("담당자삼명을 입력해주세요.");
      // this.flag=6;
      // return;
    }
    else if(this.flag6_phone==""){
      window.alert("휴대전화를 입력해주세요.");
      this.flag=6;
      return;
    }
    else if(this.flag6_store==""){
      window.alert("대표상호를 입력해주세요");
      this.flag=6;
      return;
    }
    else if(this.flag6_sub_store==""){
      window.alert("하위상호를 입력해주세요.");
      this.flag=6;
      return;
    }
    else if(this.flag6_call==""){
    ...
    else if(this.flag6_product==""){
      window.alert("제품명을 선택해주세요.");
      this.flag=6;
      return;
    }
    else if(this.flag6_model==""){
      window.alert("모델명을 선택해주세요.");
      this.flag=6;
      return;
    }
    ...
    this.firemain.child("customer").child(this.flag6_store).once("value", (sn) => {
      ...
      this.firemain.child("customer").child(this.flag6_store).child("history").child(this.flag2_receipt_num).update({
        ...
        "as_applicant"          : this.flag6_engineer,
        "as_applicant_phone"    : this.flag6_phone,
        "customer"              : this.flag6_store,
        "customer_building"     : this.flag6_sub_store,
        "address"               : this.flag6_address,
        "product_name"          : this.flag6_product,
        "product_model"         : this.flag6_model,
        "warranty"              : this.flag6_guarantee.value,
        "status"                : status,
...
ionic cordova run android --emulator
  */
  ...
  // 하단에 AS처리 버튼 클릭 후 나오는 리스트에서 상세보기로 넘어가주는 메서드
  detail(list, no) {
    console.log(list + ",," + no);
